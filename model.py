...
class ENet_model(object):
    ...
    def __init__(self, model_id, img_height=512, img_width=1024, batch_size=4):
        ...
        self.project_dir = "C:\\Users\\Chronis\\PycharmProjects\\segmentation"
    ...
    def add_logits(self):
        ...
        network, pooling_indices_1 = self.encoder_bottleneck_regular(x=network, output_depth=16,
                                                                     drop_prob=self.early_drop_prob_ph,
                                                                     scope="bottleneck_1_0", downsampling=True)
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_regular(x=network, output_depth=16,
        ...
        network, pooling_indices_2 = self.encoder_bottleneck_regular(x=network,
                                                                     output_depth=32, drop_prob=self.late_drop_prob_ph,
                                                                     scope="bottleneck_2_0", downsampling=True)
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_regular(x=network, output_depth=32,
                                                  drop_prob=self.late_drop_prob_ph, scope="bottleneck_2_1")
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_dilated(x=network, output_depth=32,
                                                  drop_prob=self.late_drop_prob_ph, scope="bottleneck_2_2",
                                                  dilation_rate=2)
        print(network.get_shape().as_list())

        network = self.encoder_bottleneck_asymmetric(x=network, output_depth=32,
